<script setup>
    ...
    const getPermissions = async () => {
        ...
        try {
            const {
                data
            } = await $fetch(URL + '/api/v1/permission', {
                method: 'GET',
            });
    ...
    const viewChildIndex = (data) => {
        // return navigateTo({ path: `/permission/${data.id}` }, { external: true }) 
        ...
        <div class="card">
            <DataTable :value="permissions" paginator :rows="5" dataKey="id" filterDisplay="menu"
                ...
                <Column header="Action" style="min-width: 12rem">
                    <template #body="{data}">
                        <Button label="View" icon="pi pi-eye" severity="primary" @click="viewChildIndex(data)" rounded
                            size="small" />
